...
func MustExist(value any, exist bool) any {
	if !exist {
		panic("must exist")
	}
	return value
...
func (m *LinkedMap[K, V]) GetFirst() (value V, exist bool) {
	return m.head.next.value, len(m.m) != 0
}

func (m *LinkedMap[K, V]) GetLast() (value V, exist bool) {
...
func (m *LinkedMap[K, V]) GetFirstEntry() *LinkedMapEntry[K, V] {
	if len(m.m) == 0 {
		return nil
	}
	return m.head.next
...
func (m *LinkedMap[K, V]) GetLastEntry() *LinkedMapEntry[K, V] {
	if len(m.m) == 0 {
		return nil
	}
	return m.tail.prev
